...
function searchPackage(input: string): Package[] {
...
function addPackage(pkg: string[], environment: string): boolean {
    let type = "";
    ...
    let disposable = vscode.commands.registerCommand('test1.searchPackage', async () => {
        ...
        quickPick.onDidChangeValue((value) => {
            ...
            debounceTimer = setTimeout(() => {
                ...
                const packageItems: PackageQuickPickItem[] = matchingPackages.map(pkg => ({
        ...
        quickPick.onDidAccept(async () => {
            const selectedPackage = quickPick.selectedItems[0] as PackageQuickPickItem;
                    ...
                    if (selectedVersion) {
                        ...
                        environmentsQP.onDidAccept(async () => {
                            ...
                            const result = await vscode.window.showInformationMessage(
                                `Are you sure you want to add "${selectedPackage.pkg.name}" package of "${selectedVersion}" version in "${selectedEnvironment}" environment?`,
                                { modal: true },
                                'Yes',
                                'No'
                            );
                            ...
                            if (result === "Yes") {
                                ...
                                if (addPackage(pkgVersion, selectedEnvironment)) {
                                    vscode.window.showInformationMessage(
                                        `Package added: ${pkgVersion} on ${selectedEnvironment}`
                                    );
                                } else {
                                    vscode.window.showErrorMessage(
                                        `Error in adding the Package: "${pkgVersion}" on ${selectedEnvironment}`
                                    );
                                }
                            } else {
